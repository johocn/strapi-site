...
export async function generateStaticParams() {
  ...
  for (const { locale } of alternateLocaleSegments()) {
    const slugs = await listGeoArticleSlugs("geo-faq", locale);
    if (slugs.length === 0) {
...
export async function generateMetadata({
  ...
  return generateGeoMetadata({ type: "geo-faq", slug, locale });
}
...
export default async function GeoFaqPage({
  ...
  const { locale: rawLocale, slug } = await params;
  const locale = normalizeLocale(rawLocale) ?? DEFAULT_LOCALE;
  return <GeoArticleView type="geo-faq" slug={slug} locale={locale} />;
}
