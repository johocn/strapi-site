...
export async function generateStaticParams() {
  ...
  for (const { locale } of alternateLocaleSegments()) {
    const slugs = await listGeoArticleSlugs("local-list", locale);
    if (slugs.length === 0) {
...
export async function generateMetadata({
  ...
  return generateGeoMetadata({ type: "local-list", slug, locale });
}

export default async function LocalListPage({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
  const { locale: rawLocale, slug } = await params;
  const locale = normalizeLocale(rawLocale) ?? DEFAULT_LOCALE;
  return <GeoArticleView type="local-list" slug={slug} locale={locale} />;
}
